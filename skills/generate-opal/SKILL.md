...
- **Temporal columns** are MANDATORY in `pick_col` whenever `pick_col` is used.
  ...
  - After `statsby`/`aggregate`, temporal columns still exist in the output — if you add `pick_col` after aggregation you MUST include them (same rule: `row_start_time()` for valid-from, `row_end_time()` for valid-to). If you omit `pick_col` entirely, temporal columns are retained automatically.
...
Time filtering is handled by the query's time range, not by OPAL. Never filter on `validFromField`/`validToField` columns — set the query's time range instead.
...
Examples: `sort desc(row_start_time())`, `make_col age:now() - row_start_time()`, `pick_col valid_from:row_start_time(), valid_to:row_end_time(), ...`.
...
1. **`pick_col` missing temporal columns.** Every `pick_col` MUST retain the dataset's temporal columns. Use `row_start_time()` for valid-from (e.g. `pick_col valid_from:row_start_time(), ...`) and `row_end_time()` for valid-to (Resource/Interval). When in doubt, omit `pick_col` entirely (all columns are kept).
2. **SQL-style sort direction.** `sort col DESC` is invalid — use `sort desc(col)`.
...
- **CRITICAL**: `pick_col` retains temporal columns whenever `pick_col` is used — including after aggregation — plus ALL `primaryKey` columns. Use `row_start_time()` for the valid-from column and `row_end_time()` for valid-to. If unsure which columns are temporal, omit `pick_col` entirely.
...
- Resource datasets use `topk` (never `limit`) and retain BOTH temporal columns in `pick_col` — `row_start_time()` for valid-from and `row_end_time()` for valid-to
